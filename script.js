...
function openAuthModal(){
  ...
  if(notice){
    notice.textContent = firebaseReady ? "Ready for Google sign-in" : "Add Firebase config first";
    notice.className = firebaseReady ? "ready" : "";
  }
...
async function signInGoogle(){
  if(!firebaseReady) {
    openAuthModal();
    alert("Firebase is not configured yet. Open firebase-config.js, paste your Firebase web config, enable Google sign-in in Firebase, then redeploy.");
    return;
  }
