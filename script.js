...
function save(){if(!state.digs)state.digs=[];localStorage.setItem('vb_v5',JSON.stringify(state))}
...
function render(){ if(!state.digs)state.digs=[]; 
...
function renderSavedGames(){
 ...
 box.innerHTML=games.length?games.map((g,i)=>`<div class="saved-game-row">
  ...
  <span>${new Date(g.savedAt).toLocaleString()}</span>
  <div>
...
document.body.addEventListener('click',e=>{
 ...
 if(e.target.dataset.deleteGame){let games=savedGames(),i=Number(e.target.dataset.deleteGame);if(confirm('Delete this saved game?')){games.splice(i,1);setSavedGames(games);renderSavedGames()}}
 ...
 if(e.target.id==='saveGameName'){let games=savedGames();if(games[editingGameIndex]){games[editingGameIndex].name=$('gameNameEdit').value.trim()||games[editingGameIndex].name;setSavedGames(games)}$('gameModal').classList.add('hidden');renderSavedGames()}
});
